...
foreach ( $foty as $f ){
    ...
    <p id="fl_<?php echo $f->id; ?>" style="margin-top: 5px;"><?php echo $f->file; ?></p>
...
<?php
    for ( $i = 1 ; $i <= $is ; ++$i ){
        if($wp == $i-1) echo '<span style="color: black;"> '.$i.' <span>';
        else echo "<a onclick=\"show_page(".($i-1).")\"; style=\"color: white; cursor: pointer; margin: 0px 5px;\">$i</a>";
    }
    ?>
</div>
