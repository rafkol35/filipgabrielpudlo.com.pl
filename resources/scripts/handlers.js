...
function uploadProgress(file, bytesLoaded) {
	...
	try {
		...
		progress.setStatus("Przesyłam plik...");
	...
	try {
		...
		//if (serverData.substring(0, 7) === "FILEID:") {
			...
			//addImage("thumbnail.php?id=" + serverData.substring(7));
                ...
                if ( serverData != '' ) {
                    alert(serverData+' jest już na serwerze. Usuń go i spróbuj ponownie!');
                    return ;
                }

                        show_files();

			progress.setStatus("Przesyłanie zakończone");
...
function uploadComplete(file) {
	...
	try {
		...
		if (this.getStats().files_queued > 0) {
			...
			progress.setStatus("Wszystkie pliki przesłane.");
			progress.toggleCancel(false);
